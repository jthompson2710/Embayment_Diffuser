...
% Anna Ruefer
% 2D diffusion of H2O and CO2 through an Embayment determined using
...
%!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    match = 2; %This is the matching of the model to instrument data. Change to 0 for H2O, 1 for CO2, 2 for both
...
%%% DIALOG INPUT VALUES
    ...
    definput = {'2D_Example_Test','3.0','500.0','500','800.0','2.0','10.0','10.0'};
    ...
    TempK = TempC+273; %temperature    init_exs = str2double(input_answer{6}); %exs content
    init_exs = str2double(input_answer{6}); %exs content
    H2Oerror = str2double(input_answer{7}); %h20 error 
    CO2error = str2double(input_answer{8}); %co2 error 
...
%%% FILL THE MESH AND DOMAIN (2D)
   ...
   %%H2O
    ...
    iH2O_C_Cry = cry_mask .* (vqH2O*0.9);
    iH2O_C = iH2O_C_Con + iH2O_C_Emb + iH2O_C_Cry  ;
   %%CO2
    ...
    iCO2_C_Cry = cry_mask .* (vqCO2*0.9);
    iCO2_C = iCO2_C_Con + iCO2_C_Emb + iCO2_C_Cry ;
%%   
%Ascent variation
%setup best match
COMP_vqH2O = vqH2O.*emb_mask_sub;
COMP_vqH2O(COMP_vqH2O==0) = [];
COMP_vqCO2 = vqCO2.*emb_mask_sub;
COMP_vqCO2(COMP_vqCO2==0) = [];
...
[H2O_C, CO2_C] = EMIDIFF_2D_OPERATOR(H2O_C,CO2_C,H2O_Cnew,CO2_Cnew, ...
    i_H2Ot,i_CO2t,MW_H2O,MW_CO2,H2OdXf,H2Od,CO2d,i_depth,dt_total,Ascent_rate,F_MDensity,GPa,...
    TGPa,Max_Time,dx,dy,dt,xq,yq,x_length,y_length,r_x,r_y,gamma,TempK,...
    H2O_emb_diff_mask,CO2_emb_diff_mask,init_exs,pre_vector_index,...
    conduit_mask,emb_mask,cry_mask,i_H2OdXf);
 
 COMP_H2O_C = H2O_C.*emb_mask_sub;
 COMP_H2O_C(COMP_H2O_C==0) = [];
 COMP_CO2_C = CO2_C.*emb_mask_sub;
 COMP_CO2_C(COMP_CO2_C==0) = [];

%determine least squares RMS match
 D_H2O_C = sqrt(mean(mean((COMP_H2O_C - COMP_vqH2O).^2))) ./ mean(mean(COMP_vqH2O));
 D_CO2_C = sqrt(sum(sum((COMP_CO2_C - COMP_vqCO2).^2))) ./ mean(mean(COMP_vqCO2));
...
%determine least squares RMS match
 D_H2O_C = sqrt(mean(mean((COMP_H2O_C - COMP_vqH2O).^2)))./ mean(mean(COMP_vqH2O));
 D_CO2_C = sqrt(mean(mean((COMP_CO2_C - COMP_vqCO2).^2)))./ mean(mean(COMP_vqCO2));
...
%%
%%%Diffusion Function
function [H2O_C, CO2_C] = EMIDIFF_2D_OPERATOR(H2O_C,CO2_C,H2O_Cnew,CO2_Cnew, ...
    i_H2Ot,i_CO2t,MW_H2O,MW_CO2,H2OdXf,H2Od,CO2d,i_depth,dt_total,Ascent_rate,F_MDensity,GPa,...
    TGPa,Max_Time,dx,dy,dt,xq,yq,x_length,y_length,r_x,r_y,gamma,TempK,...
    H2O_emb_diff_mask,CO2_emb_diff_mask,init_exs,pre_vector_index,...
    conduit_mask,emb_mask,cry_mask,i_H2OdXf)
